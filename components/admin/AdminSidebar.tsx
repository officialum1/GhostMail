...
import { BarChart2, LayoutDashboard, LogOut, Mail, Settings, Shield, Users } from 'lucide-react'
...
export default function AdminSidebar() {
  ...
  const links = [
    ...
    { name: 'Security', href: '/admin/security', icon: Shield, badge: securityAlerts > 0 ? String(securityAlerts) : null, badgeClass: 'bg-red-500/20 text-red-300 border-red-500/30' },
  ...
  return (
    <div className="w-64 bg-slate-900 border-r border-white/10 flex flex-col h-screen text-white">
      <div className="p-6 border-b border-white/10 flex items-center gap-3">
        ...
        <span className="text-[10px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full border border-blue-500/30 uppercase tracking-wider font-bold ml-auto">
          Admin
        </span>
      </div>
        ...
        {links.map((link) => {
          ...
          return (
            ...
            >
              ...
              {link.badge ? (
                <span className={`rounded-full px-2 py-0.5 text-xs border ${link.badgeClass}`}>{link.badge}</span>
              ) : null}
            </Link>
          )
        })}
