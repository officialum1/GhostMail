...
export default function TrustGrid() {
  return (
    <section className="relative border-y border-slate-200 dark:border-white/5 bg-[#0b1220]/80">
      <div className="mx-auto max-w-7xl px-6 py-14">
        <div className="grid gap-4 md:grid-cols-4">
          {publicStats.map((stat) => (
            <div key={stat.label} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-100 dark:bg-white/5 p-5">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {trustSignals.map((signal, index) => {
            ...
            return (
              <div
                key={signal.title}
                className="flex gap-4 rounded-2xl border border-slate-200 dark:border-white/10 bg-[#0e1728] p-5"
              >
