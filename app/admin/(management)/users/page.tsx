    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-2 text-slate-400">Search, moderate, export, and manage registered users.</p>
        </div>
        <button onClick={handleExport} className="rounded-2xl bg-white/10 px-4 py-3 text-sm font-medium text-white transition hover:bg-white/20">
          <span className="inline-flex items-center gap-2"><Download className="h-4 w-4" /> Export CSV</span>
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white/5 border border-white/10 rounded-2xl backdrop-blur p-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-4">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-white/10 bg-white/5 py-3 pl-11 pr-4 text-white outline-none placeholder:text-slate-500 focus:border-cyan-400"
          />
        </div>
        <select
          value={sort}
                  ...
                  <tr key={user.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 font-semibold text-white">
                    ...
                    <td className="px-6 py-4 text-slate-400">{formatRelativeDate(user.createdAt)}</td>
                    <td className="px-6 py-4">
                      <span className="rounded-full bg-cyan-400/10 px-2 py-0.5 text-xs text-cyan-300">{user._count.emails}</span>
                    </td>
                    <td className="px-6 py-4 text-slate-400">{formatRelativeDate(user.lastActive)}</td>
                    <td className="px-6 py-4">
                      ...
                      <div className="relative inline-flex justify-end">
                        ...
                        {menuOpenFor === user.id ? (
                          <div className="absolute right-0 top-12 z-20 w-52 rounded-2xl border border-white/10 bg-[#0f172a] p-2 shadow-2xl">
                            ...
                            <button onClick={() => handleResetPassword(user)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-slate-200 hover:bg-white/5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={page === 1} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-white disabled:opacity-50">
            Previous
          </button>
          <span className="text-sm text-slate-400">Page {page} / {totalPages}</span>
          <button onClick={() => setPage((value) => Math.min(totalPages, value + 1))} disabled={page >= totalPages} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-white disabled:opacity-50">
            Next
          </button>
        </div>
      </div>
      ...
      {banModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a0f1e] p-6">
            <h2 className="text-xl font-semibold text-white">{banModal.isBanned ? 'Unban user' : 'Ban user'}</h2>
            <p className="mt-2 text-sm text-slate-400">Update moderation status for {banModal.username}.</p>
            {!banModal.isBanned ? (
              <input
                value={banReason}
                onChange={(event) => setBanReason(event.target.value)}
                placeholder="Ban reason"
                className="mt-4 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-cyan-400"
              />
            ) : null}
            <div className="mt-6 flex gap-3">
              <button onClick={() => { setBanModal(null); setBanReason('') }} className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white">Cancel</button>
              <button onClick={handleBanToggle} disabled={actionLoading} className="flex-1 rounded-2xl bg-red-500 px-4 py-3 font-medium text-white disabled:opacity-60">
      ...
      {deleteModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a0f1e] p-6">
            <h2 className="text-xl font-semibold text-white">Delete {deleteModal.username}?</h2>
            <p className="mt-2 text-sm text-slate-400">This will delete user and ALL their emails. Type the username to confirm.</p>
            <input
              value={confirmUsername}
              onChange={(event) => setConfirmUsername(event.target.value)}
              placeholder={`Type "${deleteModal.username}"`}
              className="mt-4 w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none focus:border-red-400"
            />
            <div className="mt-6 flex gap-3">
              <button onClick={() => { setDeleteModal(null); setConfirmUsername('') }} className="flex-1 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white">Cancel</button>
              <button onClick={handleDelete} disabled={confirmUsername !== deleteModal.username || actionLoading} className="flex-1 rounded-2xl bg-red-500 px-4 py-3 font-medium text-white disabled:opacity-60">
      ...
      {passwordModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur">
          <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#0a0f1e] p-6">
            ...
            <p className="mt-2 text-sm text-slate-400">Copy this password now. It will only be shown once for {passwordModal.user.username}.</p>
            <div className="mt-4 rounded-2xl border border-cyan-500/30 bg-cyan-500/10 px-4 py-3 font-mono text-cyan-300">
              {passwordModal.password}
            </div>
            <div className="mt-6 flex justify-end">
              <button onClick={() => setPasswordModal(null)} className="rounded-2xl bg-white/10 px-4 py-3 text-white hover:bg-white/20">Close</button>
            </div>
          </div>
        </div>
      ) : null}
